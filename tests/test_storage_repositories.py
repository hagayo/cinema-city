...
def test_config_repository_persists_hall_layout(tmp_path: Path) -> None:
...
def test_movie_repository_persists_movies(tmp_path: Path) -> None:
...
def test_show_repository_persists_shows(tmp_path: Path) -> None:
    ...
    assert sum(len(hall.schedule.shows) for hall in loaded_cinema.halls) == 3


def test_booking_repository_persists_bookings(tmp_path: Path) -> None:
